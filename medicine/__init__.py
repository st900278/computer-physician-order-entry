...
def main():
    ...
    if 'now_med' in session:
        return render_template('index.html',
                               ...
                               patient = result,
                               past_list = past_list,
                               now_med=json.dumps(session['now_med']))
    else:
        return render_template('index.html',
                               date = datetime.date.today().strftime("%Y/%m/%d"),
                               status = session['now']+1,
...
def final():
    ...
    return render_template('prescription.html',
                           status = session['now']+1,
                           date = datetime.date.today().strftime("%Y/%m/%d"),
                           now_med = session['now_med'], alias = re)
...
def savedb():
    ...
    for x in session['now_med']:
        db.execute('insert into participant (user, start_time, end_time, patientid, symptom, med, amount, question_order) values (?, ?, ?, ?, ?, ?, ?, ?)', (session['id'], session['start-time'], strftime("%Y-%m-%d %H:%M:%S"), session['order'][session['now']], x['symptom'], x['med'], x['amount'], ','.join([str(d) for d in session['order']])))
        db.commit()
...
@app.route("/result")
def result():
    re = query_db("select * from participant")
...
@app.route("/medicine")
def med():
    medicine = query_db("select name, symptom from medicine_list")
    response = {}
    for med in medicine:
...
@app.route("/answer_med")
def answer():
    al = query_db("select symptom, name from medicine_answer")
    re = {}
    for (symptom, name) in al:
        if symptom not in re:
            re[symptom] = []
        re[symptom].append(name)
    return jsonify(ans = re)
